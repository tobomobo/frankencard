# Upstream COLDCARD README (archived)

This is Coinkite's original `README.md`, moved here verbatim when this fork
became [FRANKENCARD](README.md) so the two documents stopped competing for the
same file.

**It describes upstream COLDCARD firmware, not this fork.** Two things in it are
specifically untrue here:

- **Reproducible builds will not match Coinkite's official binaries.** This fork
  changes the firmware, so a `repro` comparison against their releases fails by
  design.
- `make -f MK4-Makefile repro` references a file that does not exist; it is
  `stm32/MK-Makefile`, and neither it nor `Q1-Makefile` actually defines a
  `repro` target.

For build instructions that work on this fork, use [TESTING.md](TESTING.md) —
it also documents the toolchain workarounds this text predates.

Coinkite's Security Advisory is **not** archived here; it is kept live at the top
of [README.md](README.md#security-advisory) because it still matters.
