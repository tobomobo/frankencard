...
effectively using Frankencard as specialized password manager.

In addition to deriving up to 10,000 distinct secure passwords, the Frankencard
can also type them into a computer by emulating a USB keyboard, and simulating the
keystrokes needed to type the password.

#### Requirements

* Frankencard Mk4 or Mk5 (firmware 5.0.5 or newer), or any Q
* USB-C with data link (won't work with power only cable from Coinkite)

## Type Passwords over USB

1. To enable "Type Passwords" feature, connect your Frankencard to host PC with USB cable (check requirements) and go to Settings -> Keyboard EMU -> Enable.
...
to exit. Exiting from "Type Passwords" will cause Frankencard to turn off keyboard emulation and enable normal USB mode if it was enabled before. Otherwise, USB stays disabled.
...
language settings. For Frankencard to be able to type the correct BIP-85
...
Passwords generated and shown on Frankencard will always be correct
...
generated and shown on Frankencard's screen.
...
Even with "non-standard" keyboard language settings, Frankencard always
...
Frankencard does not allow you to specify password length - we always
...
with bitcoin security model and therefore all passwords the Frankencard
...
Although the Frankencard is emulating a keyboard at the lowest possible level,
