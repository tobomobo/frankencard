...
Purpose: Send a small quantity of very secret data between two FRANKENCARD Q systems, with
...
- all transfers are "data typed" and decode only on FRANKENCARD
- Q model is required due to the use of QR codes to ultimately get data into the FRANKENCARD
...
being generated by the FRANKENCARD embedded firmware, it will not be
...
more than `randint`. The sending FRANKENCARD will pick a new random value each time.
...
the FRANKENCARD Q can scan from the web browser screen itself.
...
On the FRANKENCARD side, when NFC is tapped, it will offer a long URL
...
identity of your FRANKENCARD, your on chain funds or anything linked
to you.
