# Using Frankencard with Bitcoin Core
...
PSBT files for signing offline at the Frankencard.
...
Step 2: Export descriptor from Frankencard to Core

- (singlesig) on Frankencard, go to Advanced -> MicroSD card -> Export Wallet -> Bitcoin Core
- (multisig) on Frankencard, go to Settings -> Multisig Wallets -> Choose desired multisig wallet -> Descriptors -> Bitcoin Core
...
- paste the `importdescriptor` command. It should respond with a success message
  - in Bitcoin Core v24.1, the console response will include `"message": "Ranged descriptors should not have a label"` and Bitcoin Core won't allow address generation. Removing the entry `"label": "Frankencard x0x0x0x0"` from the .txt file fixes this issue.
...
First, generate a new seed phrase on the Frankencard. Then create a watch-only wallet
...
- in Bitcoin Core, go to Windows -> Console
- select Frankencard in the wallet dropdown
...
- get that onto the Frankencard, and sign it there
